...
import { loadDesignResineRowsAction, type DesignResineRow } from "./actions";
import { loadDrHistoryAction, type DrHistoryRow } from "./dr-history-actions";
...
function CaseDetailModal({ row, onClose }: { row: DrHistoryRow; onClose: () => void }) {
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#1c1c1c", border: "1px solid #333", borderRadius: 12, padding: 20, width: 420, maxHeight: "80vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, background: "#141414", border: "1px solid #222", borderRadius: 7, padding: "8px 12px" }}>
          <div style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase" as const, color: "#999", letterSpacing: "0.05em" }}>Design dents résine</div>
          {row.design_dents_resine_at ? (
            <span style={{ fontSize: 11, color: "#e0e0e0", fontWeight: 500 }}>
              {fmtDate(row.design_dents_resine_at)}
              <span style={{ marginLeft: 6, color: "#4ade80", background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.2)", borderRadius: 3, padding: "0 5px", fontWeight: 700 }}>
                {fmtTime(row.design_dents_resine_at)}
              </span>
            </span>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <Field label="Nb blocs"><Txt val={row.nb_blocs_de_dents} /></Field>
          </div>
          <div style={{ flex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <Field label="Base type"><Txt val={row.base_type} /></Field>
          </div>
          {row.commentaire_complet != null && (
            <div style={{ flex: 1 }}>
              <Field label="Commentaire"><span style={{ fontSize: 9, color: "#aaa" }}>{row.commentaire_complet}</span></Field>
            </div>
          )}
        </div>
...
function ActiveCard({ row }: { row: DesignResineRow }) {
  const natColor = NATURE_META[row.nature_du_travail ?? ""]?.color ?? "#555";
  ...
  const steps: string[] = [];
  if (dr?.design_dents_resine) steps.push("Design dents ✓");
  if (dr?.modele_a_realiser_ok) steps.push("Modèle ✓");
  if (dr?.nb_blocs_de_dents) steps.push("Blocs ✓");
  if (dr?.teintes_associees) steps.push("Teintes ✓");
  const pending: string[] = [];
  if (!dr?.design_dents_resine) pending.push("Design dents");
  if (!dr?.modele_a_realiser_ok) pending.push("Modèle");
...
function DoneCard({ row, onClick }: { row: DrHistoryRow; onClick: () => void }) {
  const natColor = NATURE_META[row.nature_du_travail ?? ""]?.color ?? "#555";
  return (
...
function DayPanel({ dateStr, rows, onSelectRow }: { dateStr: string; rows: DrHistoryRow[]; onSelectRow: (r: DrHistoryRow) => void }) {
  const d = new Date(dateStr + "T12:00:00");
...
export function DesignResineMesCas() {
  const [prodRows, setProdRows] = useState<DesignResineRow[]>([]);
  const [histRows, setHistRows] = useState<DrHistoryRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentUserId, setCurrentUserId] = useState("");
  const [detailRow, setDetailRow] = useState<DrHistoryRow | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [prod, hist] = await Promise.all([loadDesignResineRowsAction(), loadDrHistoryAction()]);
    setProdRows(prod);
    setHistRows(hist);
    setLoading(false);
  }, []);
