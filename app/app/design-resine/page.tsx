        ...
        <div style={{ display: "flex", gap: 0 }}>
          {[{ key: "production", label: "⚙ Production" }, { key: "historique", label: "📋 Historique" }].map(({ key, label }) => (
            <a key={key} href={`/app/design-resine?tab=${key}`} style={{
              ...
              color: tab === key ? "#818cf8" : "#555", marginBottom: -1, transition: "color 150ms",
            }}>{label}</a>
          ))}
        </div>
      </div>
          ...
          <div style={{ flexShrink: 0, background: "#0b0b0b", padding: "10px 20px 8px", borderBottom: "1px solid #1a1a1a" }}>
            <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
              <form action={createCaseAction} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <span style={{ fontSize: 11, color: "#818cf8", letterSpacing: 0.5 }}>Créer un cas</span>
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <button type="submit" style={{ padding: "7px 14px", border: "1px solid #818cf8", background: "rgba(129,140,248,0.08)", color: "#818cf8", cursor: "pointer", fontSize: 12, fontWeight: 700, borderRadius: 4 }}>Créer</button>
                </div>
              </form>
