...
import { loadUsinageResineRowsAction, type UsinageResineRow } from "./actions";
import { loadUrHistoryAction, type UrHistoryRow } from "./ur-history-actions";
...
function CaseDetailModal({ row, onClose }: { row: UrHistoryRow; onClose: () => void }) {
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#1c1c1c", border: "1px solid #333", borderRadius: 12, padding: 20, width: 420, maxHeight: "80vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, background: "#141414", border: "1px solid #222", borderRadius: 7, padding: "8px 12px" }}>
          ...
          {row.reception_resine_at ? (
            <span style={{ fontSize: 11, color: "#e0e0e0", fontWeight: 500 }}>
              {fmtDate(row.reception_resine_at)}
              <span style={{ marginLeft: 6, color: "#4ade80", background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.2)", borderRadius: 3, padding: "0 5px", fontWeight: 700 }}>
                {fmtTime(row.reception_resine_at)}
              </span>
            </span>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <div style={{ flex: 1 }}>
            <Field label="Machine"><Txt val={row.identite_machine} /></Field>
          </div>
          {row.identite_machine_2 && (
            <div style={{ flex: 1 }}>
              <Field label="Machine 2"><Txt val={row.identite_machine_2} /></Field>
            </div>
          )}
          <div style={{ flex: 1 }}>
            <Field label="N° disque"><Txt val={row.numero_disque} /></Field>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          {row.numero_disque_2 && (
            <div style={{ flex: 1 }}>
              <Field label="N° disque 2"><Txt val={row.numero_disque_2} /></Field>
            </div>
          )}
          <div style={{ flex: 1 }}>
            <Field label="Lot PMMA"><Txt val={row.numero_lot_pmma} /></Field>
          </div>
          <div style={{ flex: 1 }}>
            <Field label="Teintes"><Txt val={row.teintes_associees} /></Field>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <div style={{ flex: 1 }}>
            <Field label="Nb blocs"><Txt val={row.nb_blocs != null ? String(row.nb_blocs) : null} /></Field>
          </div>
          <div style={{ flex: 1 }}>
            <Field label="Modèle"><Bool val={row.modele_effectif} /></Field>
          </div>
          <div style={{ flex: 1 }}>
            <Field label="Type de dents"><Txt val={row.type_de_dents} /></Field>
          </div>
        </div>

        {/* Detail fields — Row 4 (conditional on base_type) */}
        {row.base_type && (
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <div style={{ flex: 1 }}>
              <Field label="Base"><Txt val={row.base_type} /></Field>
            </div>
            <div style={{ flex: 1 }}>
              <Field label="N° base 1"><Txt val={row.numero_base_1} /></Field>
            </div>
            <div style={{ flex: 1 }}>
              <Field label="N° base 2"><Txt val={row.numero_base_2} /></Field>
            </div>
            <div style={{ flex: 1 }}>
              <Field label="Machine base"><Txt val={row.machine_base} /></Field>
            </div>
          </div>
        )}
...
function ActiveCard({ row }: { row: UsinageResineRow }) {
  const natColor = NATURE_META[row.nature_du_travail ?? ""]?.color ?? "#555";
  ...
  const steps: string[] = [];
  if (ur?.usinage_dents_resine) steps.push("Usinage dents ✓");
  if (ur?.identite_machine) steps.push("Machine ✓");
  if (ur?.numero_disque) steps.push("Disque ✓");
  if (ur?.reception_resine_at) steps.push("Réception résine ✓");
  const pending: string[] = [];
  if (!ur?.usinage_dents_resine) pending.push("Usinage dents");
  if (!ur?.identite_machine) pending.push("Machine");

  return (
    <div style={{ background: "#1c1c1c", border: "1px solid #2a2a2a", borderRadius: 10, padding: "12px 16px", borderLeft: `3px solid ${natColor}` }}>
...
function DoneCard({ row, onClick }: { row: UrHistoryRow; onClick: () => void }) {
  const natColor = NATURE_META[row.nature_du_travail ?? ""]?.color ?? "#555";
  return (
...
function DayPanel({ dateStr, rows, onSelectRow }: { dateStr: string; rows: UrHistoryRow[]; onSelectRow: (r: UrHistoryRow) => void }) {
  const d = new Date(dateStr + "T12:00:00");
...
export function UsinageResineMesCas() {
  const [prodRows, setProdRows] = useState<UsinageResineRow[]>([]);
  const [histRows, setHistRows] = useState<UrHistoryRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentUserId, setCurrentUserId] = useState("");
  const [detailRow, setDetailRow] = useState<UrHistoryRow | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [prod, hist] = await Promise.all([loadUsinageResineRowsAction(), loadUrHistoryAction()]);
    setProdRows(prod);
    setHistRows(hist);
    setLoading(false);
  }, []);
