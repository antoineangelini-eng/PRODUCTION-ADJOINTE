...
import { loadUsinageTitaneRowsAction, type UsinageTitaneRow } from "./actions";
import { loadUtHistoryAction, type UtHistoryRow } from "./ut-history-actions";
...
// ─── H/B row (same as UsinageTitaneHistory) ─────────────────────────────────
...
function HBRow({ label, simple, h, b }: { label: string; simple: string | null; h: string | null; b: string | null }) {
  ...
  if (!hasSimple && !hasH && !hasB) return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
      <div style={{ fontSize: 9, fontWeight: 700, color: "#666", textTransform: "uppercase", letterSpacing: "0.06em", minWidth: 68 }}>{label}</div>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
      ...
      <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
        {hasSimple && <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 5, background: "#f59e0b12", border: "1px solid #f59e0b33", fontSize: 11 }}><span style={{ color: "#e0e0e0", fontWeight: 600 }}>{simple}</span></span>}
        {hasH && <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 5, background: "#5a9ba812", border: "1px solid #5a9ba844", fontSize: 11 }}><span style={{ color: "#5a9ba8", fontWeight: 800, fontSize: 9 }}>HAUT</span><span style={{ color: "#e0e0e0", fontWeight: 600 }}>{h}</span></span>}
        {hasB && <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 5, background: "#f59e0b12", border: "1px solid #f59e0b44", fontSize: 11 }}><span style={{ color: "#f59e0b", fontWeight: 800, fontSize: 9 }}>BAS</span><span style={{ color: "#e0e0e0", fontWeight: 600 }}>{b}</span></span>}
      </div>
    </div>
  );
}
...
function CaseDetailModal({ row, onClose }: { row: UtHistoryRow; onClose: () => void }) {
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#1c1c1c", border: "1px solid #333", borderRadius: 12, padding: 20, width: 420, maxHeight: "80vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          {row.sent_by_name && (
            <div>
              <div style={{ fontSize: 8, fontWeight: 600, textTransform: "uppercase" as const, color: "#999", letterSpacing: "0.05em", marginBottom: 1 }}>Envoyé par</div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          <div style={{ flex: 1 }}>
            <Field label="Teinte"><Txt val={row.teinte} /></Field>
          </div>
          <div style={{ flex: 1 }}>
...
function ActiveCard({ row }: { row: UsinageTitaneRow }) {
  const natColor = NATURE_META[row.nature_du_travail ?? ""]?.color ?? "#555";
  ...
  const isPeek = row.sector_design_metal?.peek && row.nature_du_travail === "Chassis Argoat";

  const steps: string[] = [];
  if (ut?.envoye_usinage) steps.push("Envoyé usinage ✓");
  if (ut?.reception_metal_at) steps.push("Réception métal ✓");
  if (ut?.machine_ut || ut?.machine_ut_h) steps.push("Machine ✓");
  if (ut?.numero_calcul || ut?.numero_calcul_h) steps.push("N° calcul ✓");
  const pending: string[] = [];
  if (!ut?.envoye_usinage) pending.push("Envoyé usinage");
  if (!ut?.reception_metal_at) pending.push("Réception métal");
  if (!ut?.machine_ut && !ut?.machine_ut_h) pending.push("Machine");
  if (!ut?.numero_calcul && !ut?.numero_calcul_h) pending.push("N° calcul");
...
function DoneCard({ row, onClick }: { row: UtHistoryRow; onClick: () => void }) {
  const natColor = NATURE_META[row.nature_du_travail ?? ""]?.color ?? "#555";
  return (
...
function DayPanel({ dateStr, rows, onSelectRow }: { dateStr: string; rows: UtHistoryRow[]; onSelectRow: (r: UtHistoryRow) => void }) {
  const d = new Date(dateStr + "T12:00:00");
...
export function UsinageTitaneMesCas() {
  const [prodRows, setProdRows] = useState<UsinageTitaneRow[]>([]);
  const [histRows, setHistRows] = useState<UtHistoryRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentUserId, setCurrentUserId] = useState("");
  const [detailRow, setDetailRow] = useState<UtHistoryRow | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [prod, hist] = await Promise.all([loadUsinageTitaneRowsAction(), loadUtHistoryAction()]);
    setProdRows(prod);
    setHistRows(hist);
    setLoading(false);
  }, []);
