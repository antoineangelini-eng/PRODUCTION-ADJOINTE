...
export function DesignMetalCreateBar({ onCreated, onSearch }: { onCreated?: (caseNumber: string) => void; onSearch?: (caseNumber: string) => void }) {
  ...
  async function handleCreate() {
    if (!caseNumber.trim() || !nature || !dateExp || creating) return;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button onClick={handleCreate} disabled={creating || !caseNumber.trim() || !nature || !dateExp} style={{
            padding: "7px 14px", border: "1px solid #4ade80", background: "rgba(74,222,128,0.08)",
            color: !caseNumber.trim() || !nature || !dateExp ? "#555" : "#4ade80",
